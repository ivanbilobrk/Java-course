...
/**
 * Implementation of Collection class using doubly linked list.
 *@author Ivan Bilobrk
 *@version 1.0
 */
...
public class LinkedListIndexedCollection extends Collection{
	...
	public Object[] toArray() {
		...
		class linkedListProcessor extends Processor{
			int index = 0;
			@Override
		...
		this.forEach(new linkedListProcessor());
		return array;
	}
	
	@Override
	/*
	 * Metodu forEach ostvarujemo tako da u varijablu node spremimo referencu na prvi element liste te se vrtimo u petlji sve dok 
	 * ta varijabla ne poprimi null vrijednost. U svakoj iteraciji petlje za trenutnu vrijednost čvora liste zapisane u varijabli 
	 * value zovemo metodu process, a zatim referencu node postavimo da pokazuje na sljedeći element.
	 */
	...
	public void remove(int index) {
		...
		//System.out.println(node.getValue()+" "+tempIndex);
		node.getPrevious().setNext(node.getNext());
		node.getNext().setPrevious(node.getPrevious());
		node = null;
